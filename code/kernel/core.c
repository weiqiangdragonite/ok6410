...
static void init_event_list(void);
static u8 get_bit_one(const u8 byte);
static void
remove_event_task(struct os_tcb *tcb_ptr, struct os_event *event_ptr);
...
init_os(void)
{
	...
	/* Initialize memory control list */
	init_mem_list();
}
...
/**
 *
 * Should just called inside init_os().
 *
static void
init_ready_list(void)
{
	// Clear the os_ready_group and os_ready_table
	//os_ready_group = 0;
	//memset(os_ready_table, 0, sizeof(os_ready_table));
 ...
 */
static void
init_event_list(void)
{
	int i, j;
	struct os_event *event_ptr_current;
	struct os_event *event_ptr_next;

	memset(os_event_table, 0, sizeof(os_event_table));

	for (i = 0; i < OS_MAX_EVENTS - 1; ++i) {
		j = i + 1;
		event_ptr_current = &os_event_table[i];
		event_ptr_next = &os_event_table[j];
		event_ptr_current->event_ptr = event_ptr_next;
	}
	/* Set the last event */
	event_ptr_current = &os_event_table[OS_MAX_EVENTS - 1];
...
init_tcb(prio_t prio, stk_t *stk_ptr)
{
	...
	if (tcb_ptr != NULL) {
		...
		/*
		 * prio = 30 (0001 1110) -> os_ready_table[3][6]
		 * x = prio & 7 = 0000 0110
		 * y = prio >> 3 = 0000 0011
		 * position in os_ready_table[y][x]
		 */
		tcb_ptr->tcb_x = prio & 7;
		tcb_ptr->tcb_y = prio >> 3;

		/* Set os_ready_table[y][x] = 1 */
		tcb_ptr->tcb_bit_x = 1 << tcb_ptr->tcb_x;
		/* Set the os_ready_group[y] = 1 */
		tcb_ptr->tcb_bit_y = 1 << tcb_ptr->tcb_y;
		...
		//os_ready_table[tcb_ptr->tcb_y] |= tcb_ptr->tcb_bit_x;
		//os_ready_group |= tcb_ptr->tcb_bit_y;
...
void
init_wait_event_list(struct os_event *event_ptr)
{
	event_ptr->event_prio_group = 0;

	memset(event_ptr->event_prio_table, 0,
		sizeof(event_ptr->event_prio_table));
}

/*
 * suspend task wait for event
 */
void
wait_event_task(struct os_event *event_ptr)
{
	os_tcb_current_ptr->tcb_event_ptr = event_ptr;
	event_ptr->event_prio_table[os_tcb_current_ptr->tcb_y] |=
		os_tcb_current_ptr->tcb_bit_x;
	event_ptr->event_prio_group |= os_tcb_current_ptr->tcb_bit_y;
}

prio_t
event_task_ready(struct os_event *event_ptr, u8 mask, u8 pend)
{
	u8 x, y;
	struct os_tcb *tcb_ptr;
	prio_t prio;

	y = get_bit_one(event_ptr->event_prio_group);
	x = get_bit_one(event_ptr->event_prio_table[y]);
	prio = (y << 3) + x;
	...
	/* Clear event type */
	tcb_ptr->tcb_status &= ~mask;
	tcb_ptr->tcb_pend = pend;

	/* If not suspend */
	if ((tcb_ptr->tcb_status & TASK_SUSPEND) == TASK_READY)
		tcb_ptr->tcb_status = TASK_READY;

	/* Remove this task form event list */
	remove_event_task(tcb_ptr, event_ptr);
...
{
	u8 y = tcb_ptr->tcb_y;
	event_ptr->event_prio_table[y] &= ~(tcb_ptr->tcb_bit_x);
	if (event_ptr->event_prio_table[y] == 0)
		event_ptr->event_prio_group &= ~(tcb_ptr->tcb_bit_y);

}

static u8
get_bit_one(const u8 byte)
{
	u8 i = 0;
	while ((byte & (1 << i)) != 1 && i < 8)
		++i;
	return i;
}
