...
以Pentium 4 2.0为例，它的工作主频为2.0GHz，这说明，每秒钟它会产生20亿个时钟脉冲信号，
每个时钟信号周期为0.5ns。
...
init_sys_time(void)
{
	 ...
	 * 每秒产生1 000 000 个脉冲信号
	 * 定时器时钟频率为每秒钟定时器减少值（每个时钟脉冲减1）
	 * 时钟周期为 1 / 1 MHz = 1 us，每一时钟周期减去1
	 * 定时期每 1 s 产生一次中断, TCNTB = 1 s / 1 us = 1 000 000
	 * 定时器每 10 ms 产生一次中断，TCNTB = 10 ms / 1 us = 10000
	 * 定时器每 1 ms 产生一次中断，TCNTB = 1 ms / 1 us = 1000
	 */
	TCFG0 &= ~(0xFF << 8);
	TCFG0 |= (65 << 8);
	TCFG1 = 0;
	 ...
	 * Timer 2
	 */
	TCNTB2 = (1000 / OS_TICKS_PER_SEC) * 1000;
	...
	/* 6. 设置中断服务程序ISR函数地址 */
	VIC0VECTADDR[25] = (unsigned int) timer_2_isr;
}
