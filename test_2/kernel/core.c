...
extern u32 os_start_addr;
extern u32 os_bss_start;
extern u32 os_bss_end;
...
init_os(void)
{
#if ENABLE_DEBUG
	uart_print("_start = ");
	uart_print_hex(os_start_addr);
	uart_print(", bss_start = ");
	uart_print_hex(os_bss_start);
	uart_print(", bss_end = ");
	uart_print_hex(os_bss_end);
	uart_print("\n");
#endif
	...
	/* Create memory */
	os_mem_ptr = create_mem(os_memory, NUM_BLOCK, BLOCK_LEN);
